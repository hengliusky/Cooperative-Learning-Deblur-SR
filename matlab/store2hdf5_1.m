function [curr_dat_sz, curr_lab_sz, curr_low_sz, curr_blur_sz] = store2hdf5_1(filename, data, labels, low, blur, create, startloc, chunksz)  
  ...
  if create
    ...
    h5create(filename, '/low', [low_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [low_dims(1:end-1) chunksz]); % width, height, channels, number 
    h5create(filename, '/blur', [blur_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [blur_dims(1:end-1) chunksz]); % width, height, channels, number 
    if ~exist('startloc','var') 
      startloc.dat=[ones(1,length(dat_dims)-1), 1];
      startloc.lab=[ones(1,length(lab_dims)-1), 1];
      startloc.low=[ones(1,length(low_dims)-1), 1];
      startloc.blur=[ones(1,length(blur_dims)-1), 1];
    end 
  else  % append mode
    if ~exist('startloc','var')
      ...
      prev_low_sz=info.Datasets(3).Dataspace.Size;
      prev_blur_sz=info.Datasets(4).Dataspace.Size;
      ...
      assert(prev_low_sz(1:end-1)==low_dims(1:end-1), 'Ledge dimensions must match existing dimensions in dataset');
      assert(prev_blur_sz(1:end-1)==blur_dims(1:end-1), 'Ledge dimensions must match existing dimensions in dataset');
      startloc.dat=[ones(1,length(dat_dims)-1), prev_dat_sz(end)+1];
      startloc.lab=[ones(1,length(lab_dims)-1), prev_lab_sz(end)+1];
      startloc.low=[ones(1,length(low_dims)-1), prev_low_sz(end)+1];
      startloc.blur=[ones(1,length(blur_dims)-1), prev_blur_sz(end)+1];
    end
  end
  ...
  if ~isempty(data)
    ...
    h5write(filename, '/low', single(low), startloc.low, size(low)); 
    h5write(filename, '/blur', single(blur), startloc.blur, size(blur)); 
  end
  ...
  if nargout
    ...
    curr_low_sz=info.Datasets(3).Dataspace.Size;
    curr_blur_sz=info.Datasets(4).Dataspace.Size;
  end
end
